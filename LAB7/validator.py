...
class Validator:
    ...
    def validate_name_surname(self, name_surname: str):
        ...
        """
        return bool(
            re.search("^[A-Z][a-z]{1,29}\s[A-Z][a-z]{1,29}$", name_surname))
    ...
    def validate_living_place(self, living_place: str):
        ...
        """
        return bool(
            re.search("^[A-Z][a-z]{2,19} (st.|av.|prosp.|rd.) \d(\d|[a-z])$",
                      living_place))
    ...
    def validate_phone(self, phone: str):
        ...
        """
        return bool(
            re.search("^\+\d{2}( \()?\d{3}(\) )?\d{0,3}(-?\d{2}){2}$", phone))
    ...
    def validate_id(self, given_id: str):
        ...
        """
        return bool(re.search("^((?=^\d{6}$)[1-9]{,5}0[1-9]{,5}|f)$", given_id))
    ...
    def validate(self, data: str):
        ...
        """
        functions = [self.validate_name_surname, self.validate_age,
                     self.validate_country, self.validate_region,
                     self.validate_living_place, self.validate_index,
                     self.validate_phone, self.validate_email,
                     self.validate_id]
        return [functions[func](self.split_validate(data)[func]) for func in
                range(9)] == [True] * 9
