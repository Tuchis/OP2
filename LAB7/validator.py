...
class Validator:
    ...
    def validate_age(self, age: str):
        try:
            ...
            return False
        return isinstance(age, int) and 15 < age < 100

    def validate_country(self, country: str):
        return country.isalpha() and country[0].isupper() and \
    2 <= len(country) <= 10

    def validate_region(self, region: str):
        return region[0].isupper() and \
               2 <= len(region) <= 10

    def validate_living_place(self, living_place: str):
        living_place = living_place.split()
        return living_place[0].isalpha() and living_place[0][0].isupper() \
    and 3 <= len(living_place[0]) <= 20 \
    and living_place[1] in ["st.", "av.", "prosp." ,"rd."] \
    and living_place[2][0].isnumeric() and len(living_place[2]) == 2 \
    and (living_place[2][1].isnumeric()
    or (living_place[2][1].isalpha() and living_place[2][1].islower()))

    def validate_index(self, index: str):
        return index.isnumeric() and len(index) == 5
...
if __name__ == '__main__':
    ...
    assert valid.validate(
        "Elvis Presley,20,Ukraine,Lviv,Koselnytska st. 2a,79000,+380951234567,username@domain.com,123450") is True
    assert valid.validate(
        "Elvis Presley;20;Ukraine;Lviv;Koselnytska st. 2a;79000;+380951234567;username@domain.com;123450") is True
    assert valid.validate(
        "Elvis Presley; 20; Ukraine; Lviv; Koselnytska st. 2a; 79000; +380951234567; username@domain.com; 123450") is True
